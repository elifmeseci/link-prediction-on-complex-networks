...
def sorensen_coefficient(G, ebunch=None):
    r"""Compute the Sorensen coefficient of all node pairs in ebunch.

    Sorensen coefficient of nodes `u` and `v` is defined as

    .. math::

        2 * \frac{|\Gamma(u) \cap \Gamma(v)|}{|\Gamma(u) + \Gamma(v)|}
    ...
    ebunch : iterable of node pairs, optional (default = None)
        Sorensen coefficient will be computed for each pair of nodes
    ...
    piter : iterator
        An iterator of 3-tuples in the form (u, v, p) where (u, v) is a
        pair of nodes and p is their Sorensen index.
    ...
    ..  [2] T. J. Sørensen, “A method of establishing groups of equal amplitude
            in plant sociology based on similarity of species content and its application
            to analyses of the vegetation on Danish commons”. I kommission hos E. Munksgaard, 1948.
    """
